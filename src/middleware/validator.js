...
const schemas = {
  ...
  // -------- Quotes --------
  quoteCreate(body) {
    requireFields(body, ["leadId", "venueId", "bookingType", "eventWindow", "pricing"]);
    const { QUOTE_BOOKING_TYPE, QUOTE_STATUS } = require("../config/constants");

    const leadId = validObjectId(body.leadId, "leadId");
    const venueId = validObjectId(body.venueId, "venueId");
    const bookingType = String(body.bookingType || "").trim();
    if (!Object.values(QUOTE_BOOKING_TYPE).includes(bookingType)) {
      throw new BadRequestError(`bookingType must be one of: ${Object.values(QUOTE_BOOKING_TYPE).join(", ")}`, {
        errors: [{ field: "bookingType", message: "Invalid value" }],
      });
    }

    const spaceId = body.spaceId != null && body.spaceId !== "" ? validObjectId(body.spaceId, "spaceId") : undefined;
    if (bookingType === QUOTE_BOOKING_TYPE.SPACE_BUYOUT && !spaceId) {
      throw new BadRequestError("spaceId is required for space_buyout", {
        errors: [{ field: "spaceId", message: "Required when bookingType is space_buyout" }],
      });
    }

    const eventWindow = body.eventWindow || {};
    requireFields(eventWindow, ["startAt", "endAt", "durationHours"], "Missing required fields in eventWindow");
    const durationHours = Number(eventWindow.durationHours);
    if (![12, 24, 36, 48].includes(durationHours)) {
      throw new BadRequestError("eventWindow.durationHours must be one of: 12, 24, 36, 48", {
        errors: [{ field: "eventWindow.durationHours", message: "Invalid value" }],
      });
    }
    const startAt = new Date(eventWindow.startAt);
    const endAt = new Date(eventWindow.endAt);
    if (Number.isNaN(startAt.getTime()) || Number.isNaN(endAt.getTime())) {
      throw new BadRequestError("eventWindow.startAt and eventWindow.endAt must be valid ISO dates", {
        errors: [{ field: "eventWindow", message: "Invalid date values" }],
      });
    }
    if (endAt.getTime() <= startAt.getTime()) {
      throw new BadRequestError("eventWindow.endAt must be after eventWindow.startAt", {
        errors: [{ field: "eventWindow.endAt", message: "Must be after startAt" }],
      });
    }

    const pricing = body.pricing || {};
    requireFields(pricing, ["basePrice", "gstRate", "discount", "totals"], "Missing required fields in pricing");
    const totals = pricing.totals || {};
    requireFields(
      totals,
      ["venueBase", "venueGst", "addonTotal", "addonGst", "subtotal", "discount", "total"],
      "Missing required fields in pricing.totals"
    );

    const coerceNonNegative = (value, field) => {
      const n = Number(value);
      if (!Number.isFinite(n) || n < 0) {
        throw new BadRequestError(`${field} must be a non-negative number`, {
          errors: [{ field, message: "Must be non-negative number" }],
        });
      }
      return n;
    };

    const inclusions = Array.isArray(pricing.inclusions)
      ? pricing.inclusions.map((row, idx) => {
          const name = String(row?.name || "").trim();
          if (!name) throw new BadRequestError("pricing.inclusions.name is required", { errors: [{ field: `pricing.inclusions[${idx}].name`, message: "Required" }] });
          const quantity = coerceNonNegative(row.quantity, `pricing.inclusions[${idx}].quantity`);
          const out = { name, quantity };
          if (row.maxQuantity != null && row.maxQuantity !== "") {
            out.maxQuantity = coerceNonNegative(row.maxQuantity, `pricing.inclusions[${idx}].maxQuantity`);
          }
          return out;
        })
      : [];
    ...
    const addons = Array.isArray(pricing.addons)
      ? pricing.addons.map((row, idx) => {
          const name = String(row?.name || "").trim();
          if (!name) throw new BadRequestError("pricing.addons.name is required", { errors: [{ field: `pricing.addons[${idx}].name`, message: "Required" }] });
          return {
            name,
            quantity: coerceNonNegative(row.quantity, `pricing.addons[${idx}].quantity`),
            unitPrice: coerceNonNegative(row.unitPrice, `pricing.addons[${idx}].unitPrice`),
          };
        })
      : [];

    const draft = body.draft === undefined ? true : body.draft === true || body.draft === "true";
    const confirmed = body.confirmed === undefined ? false : body.confirmed === true || body.confirmed === "true";
    if (draft && confirmed) {
    ...
    if (body.status != null && body.status !== "") {
      ...
      if (!Object.values(QUOTE_STATUS).includes(status)) {
        throw new BadRequestError(`status must be one of: ${Object.values(QUOTE_STATUS).join(", ")}`, {
          errors: [{ field: "status", message: "Invalid value" }],
        });
      }
    }
    ...
    return {
      ...
      pricing: {
        basePrice: coerceNonNegative(pricing.basePrice, "pricing.basePrice"),
        inclusions,
        addons,
        gstRate: coerceNonNegative(pricing.gstRate, "pricing.gstRate"),
        discount: coerceNonNegative(pricing.discount, "pricing.discount"),
        totals: {
          venueBase: coerceNonNegative(totals.venueBase, "pricing.totals.venueBase"),
          venueGst: coerceNonNegative(totals.venueGst, "pricing.totals.venueGst"),
          addonTotal: coerceNonNegative(totals.addonTotal, "pricing.totals.addonTotal"),
          addonGst: coerceNonNegative(totals.addonGst, "pricing.totals.addonGst"),
          subtotal: coerceNonNegative(totals.subtotal, "pricing.totals.subtotal"),
          discount: coerceNonNegative(totals.discount, "pricing.totals.discount"),
          total: coerceNonNegative(totals.total, "pricing.totals.total"),
        },
      },
      draft,
      confirmed,
  ...
  quotePatch(body) {
    ...
    if (body.bookingType !== undefined) {
      ...
      out.bookingType = bookingType;
    }
    if (body.spaceId !== undefined) {
      out.spaceId = body.spaceId ? validObjectId(body.spaceId, "spaceId") : null;
    }
    if (body.eventWindow !== undefined) {
      ...
      out.eventWindow = { startAt, endAt, durationHours };
    }
    if (body.pricing !== undefined) {
      // Patch accepts full pricing object if provided.
      const pricing = body.pricing || {};
      requireFields(pricing, ["basePrice", "gstRate", "discount", "totals"], "Missing required fields in pricing");
      const totals = pricing.totals || {};
      requireFields(
      ...
      const inclusions = Array.isArray(pricing.inclusions)
        ? pricing.inclusions.map((row, idx) => {
            ...
            if (row.maxQuantity != null && row.maxQuantity !== "") {
              item.maxQuantity = coerceNonNegative(row.maxQuantity, `pricing.inclusions[${idx}].maxQuantity`);
            }
            return item;
          })
        : [];
    ...
    if (body.draft !== undefined) {
      out.draft = body.draft === true || body.draft === "true";
    }
    if (body.confirmed !== undefined) {
      out.confirmed = body.confirmed === true || body.confirmed === "true";
    }
    if (body.status !== undefined) {
    ...
    if (Object.keys(out).length === 0) {
      throw new BadRequestError("At least one quote field is required for update");
    }
    if (out.draft === true && out.confirmed === true) {
      throw new BadRequestError("draft and confirmed cannot both be true");
    }
    return out;
  },
